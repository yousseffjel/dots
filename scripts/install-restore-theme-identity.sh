...
#
# Sourced by install-restore-theme.sh only. Assumes the caller has already
# `set -euo pipefail`, sourced global_fn.sh, and set DOTS_DIR / DRY_RUN /
# CONF_HOME plus the red/green/yellow/blue helpers.

# themes/dark/theme.conf holds the non-colour half of the theme — GTK theme
# name, icons, cursor and font. Two writers below render those same values into
# two different formats (settings.ini and xsettingsd.conf), so the reader is
...
THEME_CONF_REL="themes/dark/theme.conf"
...
# content is palette-derived. Nothing here is: all of it comes from
# themes/dark/theme.conf, which one wallpaper change does not touch. A template
...
# Add it only when themes/dark/theme.conf grows a dpi= key to read it from.
theme_write_xsettingsd_conf() {
    ...
    if [[ -e "$xs_conf" ]]; then
        if manifest_has_path THEME "$xs_conf"; then
            green "ok      $xs_conf already deployed by us"
        else
            green "ok      $xs_conf exists (left untouched, not tracked for removal)"
        fi
        return 0
    fi

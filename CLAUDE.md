...
8. **Verification is manual.** When editing package names in `install-fedora.sh`, check them against packages.fedoraproject.org — don't assume a package name is correct just because it looks plausible. Record verification method in the change log (per `session-protocol.md`).
